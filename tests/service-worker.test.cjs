...
function worker({ offline = false } = {}) {
  ...
  const caches = { async open() { return { async addAll(paths) { for (const p of paths) { const file = p === './' ? '../index.html' : '../' + p.slice(2).split('?')[0]; assert.ok(fs.existsSync(require.resolve(file))); } }, async put(...args) { puts.push(args); } }; }, async keys() { return ['stayflow-v8-cache-1', 'business-hub-v13-1', 'other-app-cache']; }, async delete(k) { removed.push(k); }, async match(request) { return typeof request === 'string' ? cached : undefined; } };
...
test('offline navigation serves the cached shell but a missing asset does not return HTML', async () => {
  ...
  const asset = worker({ offline: true }); asset.handlers.fetch(asset.event({ method: 'GET', url: 'https://example.com/stayflow/app.js?v=13', mode: 'cors' })); assert.equal((await asset.pending[0]).type, 'error');
});
